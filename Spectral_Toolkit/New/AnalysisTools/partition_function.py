...
def partition_function(element, ion_state,Tev):
    ...
    #read from data
    df = pd.read_csv('AnalysisTools/Databases/partition_data/' +element+" "+ ion_state+'.txt',delimiter=',',index_col=False)
...
def test_partition_function():
    ...
    df = pd.read_csv('AnalysisTools/Databases/element_data/ion_data.csv',delimiter=';',index_col=False)
    ...
    for i1 in range(0,len(element_list)):
        ...
        for j1 in range(0,len(element_ions[i1])):
            ...
            current_file = open('AnalysisTools/Databases/partition_data/' +element+" "+ ion_state+'.txt','r')
